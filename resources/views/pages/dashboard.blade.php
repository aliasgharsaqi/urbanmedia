    ...
    <div>
        ...
        @if($events->isEmpty())
            <div class="bg-white p-6 sm:p-8 rounded-xl shadow-lg text-center">
                ...
                <a href="{{ route('events.create') }}" class="mt-4 inline-block bg-orange-500 hover:bg-orange-600 text-white font-semibold px-4 py-2 sm:px-6 sm:py-2 rounded-lg transition duration-200 text-sm sm:text-base">
                    Create an Event
                </a>
            </div>
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                ...
                <div class="bg-white rounded-xl shadow-lg overflow-hidden transform hover:-translate-y-1 transition-transform duration-300">
                    <div class="p-4 sm:p-5">
                        ...
                        <h3 class="text-base sm:text-lg font-bold text-gray-800 truncate">{{ $event->heading }}</h3>
                        <p class="text-xs sm:text-sm text-gray-600 mt-1">Client: {{ $event->client_name }}</p>
                    </div>
                    <div class="bg-gray-50 px-4 sm:px-5 py-3">
                        <a href="{{ route('events.show', $event) }}" class="text-xs sm:text-sm font-semibold text-orange-600 hover:text-orange-700">View Details &rarr;</a>
                    </div>
                </div>
                @endforeach
            </div>
