        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="md:col-span-1">
                @if ($event->event_image)
                    <img src="{{ Storage::url($event->event_image) }}" alt="Event Image" class="w-full h-auto rounded-lg shadow-md mb-6">
                @else
                    <div class="w-full h-48 bg-gray-200 rounded-lg flex items-center justify-center text-gray-500 mb-6">
                        No Image Provided
                    </div>
                @endif
                <h3 class="text-lg font-bold text-gray-800 border-b pb-2 mb-3">Description</h3>
                <p class="text-gray-600 text-sm leading-relaxed">{{ $event->desc ?? 'No description provided.' }}</p>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5 text-sm">
                    ...
                    <div>
                        ...
                        <p class="text-gray-800 capitalize">{{ $event->status ?? 'N/A' }}</p>
                    </div>
                    
                    <div>
                        <p class="font-semibold text-gray-500">Event Access</p>
                        <p class="text-gray-800 capitalize">{{ $event->event_access_type ?? 'N/A' }}</p>
                    </div>
                    ...
                    <div>
                        <p class="font-semibold text-gray-500">Venue</p>
                        <p class="text-gray-800">{{ $event->venue ?? 'N/A' }}</p>
                    </div>

                    <div>
                        <p class="font-semibold text-gray-500">City</p>
                        <p class="text-gray-800">{{ $event->city ?? 'N/A' }}</p>
                    </div>

                    <div>
                        <p class="font-semibold text-gray-500">Price</p>
                        <p class="text-gray-800">{{ $event->price ? '₹' . number_format($event->price, 2) : 'N/A' }}</p>
                    </div>

                    <div>
                        <p class="font-semibold text-gray-500">Expected Guests</p>
                        <p class="text-gray-800">{{ $event->expected_guest ?? 'N/A' }}</p>
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        <p class="font-semibold text-gray-500">Street Address</p>
                        <p class="text-gray-800">{{ $event->address ?? 'N/A' }}</p>
                    </div>
                    ...
                    <div class="sm:col-span-2 mt-4">
                        <h3 class="text-lg font-bold text-gray-800 border-b pb-2 mb-3">Terms & Conditions</h3>
